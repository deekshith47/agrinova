...
import { getFinancialMarketData } from '../services/geminiService';
...
const TrendIndicator = ({ trend }: { trend: 'Rising' | 'Stable' | 'Falling' }) => {
    const trendConfig = {
        Rising: { text: '▲ Rising', color: 'text-green-600', bg: 'bg-green-100' },
        Stable: { text: '— Stable', color: 'text-slate-600', bg: 'bg-slate-100' },
        Falling: { text: '▼ Falling', color: 'text-red-600', bg: 'bg-red-100' },
    };
    const { text, color, bg } = trendConfig[trend];
    return <span className={`px-3 py-1 text-sm font-bold rounded-full ${color} ${bg}`}>{text}</span>;
}

const PriceChart = ({ data }: { data: { month: string, price: number }[] }) => {
    ...
    return (
        <div className="p-4 bg-slate-50 rounded-lg h-full">
             <h4 className="font-semibold text-slate-700 mb-4 text-center">3-Month Price Trend (₹)</h4>
...
const PieChart = ({ data }: { data: { crop: string, percentage: number }[] }) => {
    if (!data || data.length === 0) return null;
    ...
    return (
        <div className="p-4 bg-slate-50 rounded-lg h-full">
            <h4 className="font-semibold text-slate-700 mb-4 text-center">Projected Revenue Contribution</h4>
...
export const FinancialDashboard: React.FC = () => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 h-full">
            <h2 className="text-2xl font-bold text-brand-dark mb-4">Financial & Market Prices</h2>
            
            <div className="max-w-md">
                <label htmlFor="crop-select" className="block text-sm font-medium text-slate-700">Select a Crop to Analyze</label>
                <select
                    id="crop-select"
                    value={selectedCrop}
            ...
            {isLoading && (
                <div className="flex items-center justify-center h-64">
                    ...
                    <p className="ml-4 text-slate-500">Fetching latest market data...</p>
                </div>
            )}
            ...
            {!isLoading && !financialData && !error && (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-xl font-bold text-brand-dark">Market Insights Await</h3>
                    <p className="text-slate-500 mt-2 max-w-sm">
                        Select a crop from the dropdown above to get the latest AI-powered financial analysis, price trends, and a revenue contribution pie chart.
                    </p>
                </div>
            )}
                ...
                <div className="mt-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-green-50 p-4 rounded-lg">
                            <p className="text-sm font-medium text-green-800">Current Market Price ({financialData.currentPrice.source})</p>
                            <p className="text-3xl font-bold text-green-900">
                                ₹{financialData.currentPrice.price.toLocaleString('en-IN')}
                                <span className="text-lg font-medium text-green-700"> /{financialData.currentPrice.unit}</span>
                            </p>
                        </div>
                        <div className="bg-slate-50 p-4 rounded-lg flex flex-col justify-center items-center">
                            <p className="text-sm font-medium text-slate-600 mb-2">Market Trend</p>
                            <TrendIndicator trend={financialData.priceTrend} />
                        </div>
                    </div>
                    
                    <div>
                        <h4 className="font-semibold text-slate-700">AI Market Insights</h4>
                        <p className="text-sm text-slate-600 mt-2 whitespace-pre-wrap">{financialData.marketInsights}</p>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <PriceChart data={financialData.historicalData} />
                        <PieChart data={financialData.revenueContribution} />
                    </div>
                </div>
            )}
        </div>
